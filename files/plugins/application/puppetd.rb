class MCollective::Application::Puppetd<MCollective::Application
    ...
    def post_option_parser(configuration)
        if ARGV.length >= 1
            ...
            unless configuration[:command].match(/^(enable|disable|runonce|noop|runall|status|summary|count)$/)
                raise "Command has to be enable, disable, runonce, runonce, noop, runall, status, summary or count"
            end
        else
    ...
    def main
        ...
        case configuration[:command]
            ...
            when "noop"
                printrpc mc.noop(:forcerun => configuration[:force])
